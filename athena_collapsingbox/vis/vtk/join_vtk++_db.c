...
static void init_domain_1d(void){
  ...
  for(i=0; i<file_count; i++){
    ...
    if(grid_version == 1){ // fixed linear grids in athena 4.2
      ...
      fscanf(fp,"X_COORDINATES %d double\n", &nx);
      printf("X_COORDINATES %d double\n", nx);
      domain_1d[i].X = (double*)malloc(nx*sizeof(double));
      for(j=0; j<nx; ++j){
        if((nread = fread(&fcoord, sizeof(double), 1, fp)) != 1)
                  join_error("reading X_COORDINATES error\n");
	if(!big_end) Swap8Bytes(&fcoord);
	domain_1d[i].X[j] = fcoord;
      }

      /* Y_COORDINATES NY double */
      fscanf(fp,"\nY_COORDINATES %d double\n", &ny);
      printf("Y_COORDINATES %d double\n", ny);
      domain_1d[i].Y = (double*)malloc(ny*sizeof(double));
      for(j=0; j<ny; ++j){
        if((nread = fread(&fcoord, sizeof(double), 1, fp)) != 1)
	...
	domain_1d[i].Y[j] = fcoord;
      ...
      printf("Z_COORDINATES %d double\n", nz);
      domain_1d[i].Z = (double*)malloc(nz*sizeof(double));
      for(j=0; j<nz; ++j){
        if((nread = fread(&fcoord, sizeof(double), 1, fp)) != 1)
	...
	for(j=0; j<domain_3d[0][jg][0].Ny; j++){
	  for(ig=0; ig<NGrid_x; ig++){
	    for(i=0; i<domain_3d[0][0][ig].Nx; i++){
	      ...
	      if((nread = fread(&fdat, sizeof(double), 1, fp)) != 1)
		join_error("read_write_scalar error\n");

	      fwrite(&fdat, sizeof(double), 1, fp_out);
	    }
	  }
	}
      }
	...
	for(j=0; j<domain_3d[0][jg][0].Ny; j++){
	  for(ig=0; ig<NGrid_x; ig++){
	    for(i=0; i<domain_3d[0][0][ig].Nx; i++){
	      ...
	      if((nread = fread(fvec, sizeof(double), 3, fp)) != 3)
		join_error("read_write_vector error\n");

	      fwrite(fvec, sizeof(double), 3, fp_out);
	    }
	  }
	}
      }
	...
	for(j=0; j<domain_3d[0][jg][0].Ny; j++){
	  for(ig=0; ig<NGrid_x; ig++){
	    for(i=0; i<domain_3d[0][0][ig].Nx; i++){
	      ...
	      if((nread = fread(ftensor, sizeof(double), 9, fp)) != 9)
		join_error("read_write_vector error\n");

	      fwrite(ftensor, sizeof(double), 9, fp_out);
	    }
	  }
	}
      }
...
static void write_joined_vtk(const char *out_name){
  ...
  if(grid_version == 1){
    ...
    fprintf(fp_out,"DIMENSIONS %d %d %d\n", nxp, nyp, nzp);
    /* Write X Coords*/
    fprintf(fp_out,"X_COORDINATES %d double\n",nxp);
    for(i=0; i<NGrid_x; i++)
      for(ii=0; ii<domain_3d[0][0][i].Nx; ++ii){
        fcoord=domain_3d[0][0][i].X[ii];
        if(!big_end) Swap8Bytes(&fcoord);
        fwrite(&fcoord, sizeof(double), 1, fp_out);
      }
	...
	for(i=0; i<NGrid_x; i++){
	  if(i == 0 && j == 0 && k == 0){
	    ...
	    if(strcmp(format, "double") != 0){
	      fclose(fp_out);
	      join_error("Expected \"double\" format, found \"%s\"\n",format);
	    }
	  }
	  else{
	    retval = fscanf(domain_3d[k][j][i].fp,"%s %s %s\n",
